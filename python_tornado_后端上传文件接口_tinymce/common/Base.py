...
class MyPymysql(base_pymysql):
    ...
    def __init__(self, conf_name, cursor=pymysql.cursors.DictCursor):
    ...
    def idu_sql_test(self, sql, value=None):
        # adu: insert, delete, update的简写
        # 考虑到多语句循环, try就不写在这里了
        self.cursor.execute(sql, value)
...
def result(status, value=None, messageValue=None):
    ...
    return json.dumps({
        "statuscode": status,
        "statusmessage": my_message[status],
        "value": value
...
class CustomException(Exception):
    ...
    def __init__(self, status, msg='', des=None):
        if not msg:
            try:
                msg = my_message[status]
            except:
                msg = "An unexpected mistake！！！"
        self.message = '({}, {}, {})'.format(status, msg, des)
